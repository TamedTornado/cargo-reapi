...
if [ "$#" -ne 7 ]; then
  echo "usage: $0 REPORT_ROOT BATCH_ID BRO_ROOT MORIA_ROOT CACHE_DIR PRODUCER_RETIREMENT STORAGE_PROFILE" >&2
  exit 2
fi
...
moria_root=$(cd "$4" && pwd)
cache_dir=$(cd "$5" && pwd)
producer_retirement=$(cd "$(dirname "$6")" && pwd)/$(basename "$6")
storage_profile=$7
...
jq -n \
  ...
  '{schema_version:1,observer_kind:$observer_kind,observer_version:$observer_version,observer_command:["Bro public PM script under OS observer"],selected_executables:[{path:$rustc,sha256:$rustc_sha},{path:$linker,sha256:$linker_sha}],expected:"zero",attribution_root:null,expected_crates:[],coalescing_roots:[]}' \
  >"$report_root/consumers-selection.json"
...
if [ "$platform" = macos ]; then
  ...
  sleep 1
  kill -0 "$observer_pid"
  run_bro >"$report_root/bro-cli.stdout" 2>"$report_root/bro-cli.stderr" || status=$?
  kill -TERM "$observer_pid" 2>/dev/null || true
  wait "$observer_pid" 2>/dev/null || true
else
  events=$report_root/consumers-os-events.strace
  # Invoke the public package script directly so strace owns the full Bro process tree.
...
claims=$(jq -cn '
  ...
  {public_cli_boundary:claim(["bro_cli_stdout","bro_harness_source"]),five_jobs_simultaneous:claim(["bro_proof"]),canonical_gate:claim(["bro_proof","member_action_log"]),zero_physical_actions:claim(["bro_proof","member_action_log"]),zero_os_compiler_linker:claim(["bro_os_audit"]),deadline_met:claim(["bro_proof"])}')
references="bro_proof:$report_root/bro-moria-five-proof.json bro_population_proof:$report_root/bro-moria-five-population-proof.json bro_os_audit:$report_root/consumers-os-proof.json producer_retirement:$producer_retirement bro_cli_stdout:$report_root/bro-cli.stdout bro_cli_stderr:$report_root/bro-cli.stderr bro_harness_source:$report_root/bro-harness-source.ts bro_source_revision:$report_root/bro-source-revision.txt"
